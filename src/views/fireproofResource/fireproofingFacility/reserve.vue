  ...
  <div class="app-list-page" v-loading="requestLoading">
      ...
      <div class="screen-box-button">
        <el-popover placement="bottom" width="480" trigger="click">
          ...
          <div slot="reference" style="display: flex; align-items: center">
            <div v-permission="['fireprot:reserve:more']" class="text1">
              更多
        ...
        <el-button
          type="primary"
          class="btn"
          v-permission="['fireprot:reserve:query']"
    ...
    <div class="operation-box">
      <div class="operation-box-left">
        <el-button
          v-permission="['fireprot:reserve:create']"
          class="btn1"
          type="primary"
          icon="el-icon-addnew"
          plain
        ...
        >
        <el-button
          v-permission="['fireprot:reserve:delete']"
          class="btn2"
          type="danger"
        ...
        >
        <el-button
          v-permission="['fireprot:reserve:import']"
          class="btn3"
        ...
        >
        <el-button
          v-permission="['fireprot:reserve:export']"
          class="btn4"
        ...
        >
        <el-button
          v-permission="['fireprot:reserve:map-plus']"
          class="btn5"
          icon="el-icon-earth"
          plain
          >一张图</el-button
      ...
      <div class="operation-box-right">
        ...
        >
          ...
          <div
            slot="reference"
            v-permission="['fireprot:reserve:column-filter']"
        ...
        <div
          v-permission="['fireprot:reserve:layout']"
          style="display: flex; align-items: center"
    ...
    <div class="list-map-box" :class="[isMoveBox ? 'not-select' : '']">
      ...
      >
        ...
        >
          ...
          <el-table-column
            v-if="checkedTh.includes('establishmentTime')"
          ...
          <el-table-column
            v-if="checkedTh.includes('buildArea')"
            prop="buildArea"
            label="建设面积(平方米)"
            min-width="120"
            show-overflow-tooltip
          ...
          <el-table-column
            v-if="checkedTh.includes('reserveLevel')"
            prop="reserveLevel"
            label="储备等级"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('capacity')"
            prop="capacity"
            label="有效容量(立方米)"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('mgmtUnit')"
            prop="mgmtUnit"
            label="管理单位"
            min-width="120"
            show-overflow-tooltip
          ...
          >
            <template slot-scope="scope" style="display: flex">
              <el-button
                size="mini"
                type="primary"
                @click="handleSuppliesClick(scope.row)"
                >物资</el-button
              >
              <el-button
                size="mini"
                type="primary"
                @click="handleManagerClick(scope.row)"
                >管理人员</el-button
              >
              <i
    ...
    <ReserveEditDialog
      ...
      @handleSuppliesClick="handleSuppliesClick"
      @handleManagerClick="handleManagerClick"
    />

    <el-dialog
      title="储备物资管理"
      :visible.sync="dialogVisibleSupplies"
      class="dialog-class"
      width="84%"
      top="50px"
    >
      <ReserveSupplies
        v-if="dialogVisibleSupplies"
...
import ReserveEditDialog from "./editDialogs/reserveEditDialog.vue";
import ReserveEdit from "./edits/reserveEdit.vue";

import ReserveSupplies from "./supplies.vue";
import ReserveManager from "./reserveManager.vue";
...
export default {
  ...
  data() {
    return {
      ...
      checkAll: false,
      checkedTh: fireproofDict("fireprot_reserve").showColumn,
      ...
      tableListTitle: [
        ...
        { name: "成立时间", parameter: "establishmentTime" },
        { name: "建设面积(平方米)", parameter: "buildArea" },
        { name: "储备等级", parameter: "reserveLevel" },
        { name: "有效容量(立方米)", parameter: "capacity" },
        { name: "管理单位", parameter: "mgmtUnit" },
  ...
  methods: {
    handleSuppliesClick(row) {
      this.suppliesId = row.id;
      this.dialogVisibleSupplies = true;
    },
    handleManagerClick(row) {
    ...
    // 列表数据查询
    getList() {
      reserveApi("page", "post", this.queryData)
        .then((res) => {
    ...
    async handleMoveSelectClick(type) {
      ...
      this.$nextTick(() => {
        this.$refs["reserveEdit"].updateDetails();
      });
    },
    ...
    async tableCurrentChange(currentRow) {
      ...
      if (this.isScreenCutting) {
        ...
        this.$nextTick(() => {
          this.$refs["reserveEdit"].initDetails();
        });
      }
    },
    ...
    downExcelClick() {
      ...
      reserveApi("export_all/excel", "post", data).then((res) => {
    ...
    getDeleteFunction(data) {
      ...
      this.$confirm("确定删除？", "提示", {
        ...
        .then(() => {
          reserveApi("delete", "post", ids).then((res) => {
            this.$message({
              type: "success",
              message: "删除成功",
            });
