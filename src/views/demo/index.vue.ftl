  ...
  <div class="app-list-page" v-loading="requestLoading">
    ...
    <div class="screen-box">
      ...
      <#list queryList as field>
        ...
        <#elseif field.queryFormType == 'text'>
        <div class="screen-box-item">
          <div class="item-text">${field.fieldComment}</div>
          <el-input
            v-model.trim="query.${field.attrName}"
            style="flex: 1"
      ...
      <div class="screen-box-button">
       <el-popover placement="bottom" width="480" trigger="click">
          ...
          >
            <div v-permission="['${moduleName}:${functionName}:more']" class="text1">
              更多
        ...
        <el-button
          type="primary"
          class="btn"
          v-permission="['${moduleName}:${functionName}:query']"
    ...
    <div class="operation-box">
      <div class="operation-box-left">
        <el-button
          v-permission="['${moduleName}:${functionName}:create']"
          class="btn1"
          type="primary"
          icon="el-icon-addnew"
          plain
        ...
        >删除</el-button>
        <el-button
          v-permission="['${moduleName}:${functionName}:import']"
          class="btn3"
          icon="el-icon-importnew"
          plain
        >导入</el-button>
        <el-button
          v-permission="['${moduleName}:${functionName}:export']"
          class="btn4"
        ...
        >导出</el-button>
        <el-button
          v-permission="['${moduleName}:${functionName}:map-plus']"
          class="btn5"
          icon="el-icon-earth"
          plain
        >一张图</el-button>
      ...
      <div class="operation-box-right">
        ...
        >
          ...
          <div
            slot="reference"
            v-permission="['${moduleName}:${functionName}:column-filter']"
            style="display: flex; align-items: center"
          >
        ...
        <div
          v-permission="['${moduleName}:${functionName}:layout']"
          style="display: flex; align-items: center"
      ...
      >
        ...
        >
          ...
          <#list gridList as field>
            ...
            <!--  这个是表格那一列能够点击展示详情弹框的  -->
            <#elseif field.fieldName == email && field.queryFormType != 'select'>
            <el-table-column
              v-if="checkedTh.includes('${field.attrName}')"
              prop="${field.attrName}"
            ...
            >
              <template slot-scope="scope">
                <div class="c-click" @click="handleDetailsClick(scope.row)">
                  {{ scope.row.${field.attrName} }}
                </div>
              </template>
            </el-table-column>
            <!--  这个是表格那一列能够点击展示详情弹框的并且是select  -->
            <#elseif field.fieldName == email && field.queryFormType == 'select'>
            <el-table-column
              v-if="checkedTh.includes('${field.attrName}')"
              prop="${field.attrName}"
            ...
            >
              <template slot-scope="scope">
                <div class="c-click" @click="handleDetailsClick(scope.row)">
                  <dictionary-text code="${field.formDict}" :value="scope.row.${field.attrName}" />
                </div>
              </template>
            </el-table-column>
            <!--  这个是创建是有有升序降序操作的列  -->
            <#elseif field.gridSort && field.queryFormType != 'select'>
            <el-table-column
              v-if="checkedTh.includes('${field.attrName}')"
              prop="${field.attrName}"
              label="${field.fieldComment}"
            ...
            <#elseif field.queryFormType == 'select'  && !field.gridSort>
            <el-table-column
              v-if="checkedTh.includes('${field.attrName}')"
              prop="${field.attrName}"
              label="${field.fieldComment}"
...
import ${FunctionName}EditDialog from './editDialogs/${functionName}EditDialog.vue';
import ${FunctionName}Edit from './edits/${functionName}Edit.vue';
...
export default {
  ...
  data() {
    return {
      ...
      checkAll: false,
      checkedTh: fireproofDict("${tableName}").showColumn,
      isIndeterminate: true,
      // 表格表头列表
      tableListTitle: [
        <#list gridList as field>
        <#if field.attrName != 'id'>
        { name: '${field.fieldComment}', parameter: '${field.attrName}' },
        </#if>
        </#list>
      ]
    };
  ...
  methods: {
    ...
    // 列表数据查询
    getList() {
      ${functionName}Api('page', 'post', this.queryData)
        .then((res) => {
    ...
    async handleMoveSelectClick(type) {
      ...
      this.$nextTick(() => {
        this.$refs['${functionName}Edit'].updateDetails();
      });
    },
    ...
    async tableCurrentChange(currentRow) {
      ...
      if (this.isScreenCutting) {
        ...
        this.$nextTick(() => {
          this.$refs['${functionName}Edit'].initDetails();
        });
      }
    },
    ...
    downExcelClick() {
      ...
      ${functionName}Api("export_all/excel", "post", data).then((res) => {
    ...
    getDeleteFunction(data) {
      ...
      this.$confirm('确定删除？', '提示', {
        ...
        .then(() => {
          ${functionName}Api('delete', 'post', ids).then((res) => {
            this.$message({
              type: 'success',
              message: '删除成功'
            });
