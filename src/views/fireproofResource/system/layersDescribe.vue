  ...
  <div v-loading="requestLoading" class="app-list-page">
    ...
    <div class="screen-box">
      ...
      <div class="screen-box-input">
        ...
        <!-- <div class="screen-box-item">
          <div class="item-text">图层描述</div>
          <el-input
            v-model.trim="query.describe"
            style="flex: 1"
            placeholder="请输入图层描述"
            class="filter-item"
            clearable
    ...
    <div class="list-map-box" :class="[isMoveBox ? 'not-select' : '']">
      ...
      >
        ...
        >
          ...
          <el-table-column
            v-if="checkedTh.includes('lastModifyTime')"
            prop="lastModifyTime"
            label="修改时间"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('layerDetailName')"
            prop="layerDetailName"
            label="图层详细名"
            min-width="120"
            show-overflow-tooltip
          >
          ...
          <el-table-column
            v-if="checkedTh.includes('timing')"
            prop="timing"
            label="时间段"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('source')"
            prop="source"
            label="数据来源"
            min-width="120"
            sortable="custom"
...
import LayersDescribeEditDialog from "./editDialogs/layersDescribeEditDialog.vue";
...
export default {
  ...
  data() {
    return {
      ...
      checkAll: false,
      checkedTh: ["layerDetailName", "describe", "source"],
      isIndeterminate: true,
      // 表格表头列表
      tableListTitle: [
        { name: "创建时间", parameter: "createdTime" },
        { name: "修改时间", parameter: "lastModifyTime" },
        { name: "图层详细名", parameter: "layerDetailName" },
        { name: "时间段", parameter: "timing" },
        { name: "数据来源", parameter: "source" },
      ],
    };
  },
  ...
  methods: {
    ...
    // 列表数据查询
    getList() {
      layersDescribeApi("page", "post", this.queryData)
        .then((res) => {
    ...
    getDeleteFunction(data) {
      ...
      this.$confirm("确定删除？", "提示", {
        ...
        .then(() => {
          layersDescribeApi("delete", "post", ids).then((res) => {
            this.$message({
              type: "success",
              message: "删除成功",
            });
