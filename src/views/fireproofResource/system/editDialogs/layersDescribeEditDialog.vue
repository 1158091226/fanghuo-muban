      ...
      >
        ...
        >
          ...
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.layerDetailName"
              style="width: 100%"
              placeholder="请输入图层详细名"
              class="filter-item"
              clearable
            />
            <div v-else>{{ formDetailsDialog.layerDetailName }}</div>
          </el-form-item>
          <el-form-item
            label="图层描述"
            :class="[
          ...
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.timing"
              style="width: 100%"
              placeholder="请输入时间段"
              class="filter-item"
              clearable
            />
            <div v-else>{{ formDetailsDialog.timing }}</div>
          </el-form-item>
          <el-form-item
            label="数据来源"
            :class="[
          ...
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.source"
              style="width: 100%"
              placeholder="请输入数据来源"
              class="filter-item"
              clearable
            />
            <div v-else>{{ formDetailsDialog.source }}</div>
          </el-form-item>
        </el-form>
      </div>
  ...
  methods: {
    ...
    getDetailsFunction(queryData, listdata) {
      return new Promise((resolve, reject) => {
        layersDescribeApi("detail", "get", { id: queryData.id })
          .then((res) => {
            let data = {};
    ...
    handleSubmitClick() {
      this.$refs["formDetailsDialog"].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogtitle == "新增") {
            layersDescribeApi("create", "post", this.formDetailsDialog)
              .then((res) => {
                this.$message({
                  type: "success",
                  message: "新增成功",
            ...
            layersDescribeApi("update", "post", data)
              .then((res) => {
                this.$message({
                  type: "success",
                  message: "修改成功",
