  ...
  <div class="app-list-page" v-loading="requestLoading">
    ...
    <div class="screen-box">
      ...
      <div class="screen-box-input">
        ...
        <div class="screen-box-item">
          <div class="item-text">保护地类型</div>
          <dictionary-select
            v-model.trim="query.protectType"
        ...
        <div class="screen-box-item">
          <div class="item-text">保护等级</div>
          <dictionary-select
            v-model.trim="query.protectLevel"
      ...
      <div class="screen-box-button">
        <el-popover placement="bottom" width="480" trigger="click">
          ...
          <div slot="reference" style="display: flex; align-items: center">
            <div v-permission="['forestry:naturalReserve:more']" class="text1">
              更多
        ...
        <el-button
          type="primary"
          class="btn"
          v-permission="['forestry:naturalReserve:query']"
    ...
    <div class="operation-box">
      <div class="operation-box-left">
        <el-button
          v-permission="['forestry:naturalReserve:create']"
          class="btn1"
          type="primary"
          icon="el-icon-addnew"
          plain
        ...
        >
        <el-button
          v-permission="['forestry:naturalReserve:import']"
          class="btn3"
        ...
        >
        <el-button
          v-permission="['forestry:naturalReserve:export']"
          class="btn4"
        ...
        >
        <el-button
          v-permission="['forestry:naturalReserve:map-plus']"
          class="btn5"
          icon="el-icon-earth"
          plain
          >一张图</el-button
      ...
      <div class="operation-box-right">
        ...
        >
          ...
          <div
            slot="reference"
            v-permission="['forestry:naturalReserve:column-filter']"
        ...
        <div
          v-permission="['forestry:naturalReserve:layout']"
          style="display: flex; align-items: center"
    ...
    <div class="list-map-box" :class="[isMoveBox ? 'not-select' : '']">
      ...
      >
        ...
        >
          ...
          <el-table-column
            v-if="checkedTh.includes('protectTarget')"
            prop="protectTarget"
            label="保护对象"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('protectType')"
            prop="protectType"
            label="保护地类型"
            min-width="120"
            sortable="custom"
          ...
          >
            <template slot-scope="scope">
              <dictionary-text
                ...
                :value="scope.row.protectType"
              />
            </template>
          </el-table-column>

          <el-table-column
            v-if="checkedTh.includes('protectLevel')"
            prop="protectLevel"
            label="保护等级"
            min-width="120"
            sortable="custom"
          ...
          >
            <template slot-scope="scope">
              <dictionary-text
                ...
                :value="scope.row.protectLevel"
              />
            </template>
          </el-table-column>

          <el-table-column
            v-if="checkedTh.includes('plotArea')"
            prop="plotArea"
            label="落图面积(亩)"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('coreArea')"
            prop="coreArea"
            label="核心区面积(亩)"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('bufferArea')"
            prop="bufferArea"
            label="缓冲区面积(亩)"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('approvedArea')"
            prop="approvedArea"
            label="审批面积(亩)"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('approvedTime')"
            prop="approvedTime"
            label="批建时间"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('investigator')"
            prop="investigator"
            label="调查人"
            min-width="120"
            sortable="custom"
...
import NaturalReserveEditDialog from "./editDialogs/naturalReserveEditDialog.vue";
import NaturalReserveEdit from "./edits/naturalReserveEdit.vue";
...
export default {
  ...
  data() {
    return {
      ...
      checkAll: false,
      checkedTh: fireproofDict("forestry_natural_reserve").showColumn,
      isIndeterminate: true,
      // 表格表头列表
      tableListTitle: [
        { name: "区划代码", parameter: "areaCode" },
        { name: "名称", parameter: "name" },
        { name: "保护对象", parameter: "protectTarget" },
        { name: "保护地类型", parameter: "protectType" },
        { name: "保护等级", parameter: "protectLevel" },
        { name: "落图面积(亩)", parameter: "plotArea" },
        { name: "核心区面积(亩)", parameter: "coreArea" },
        { name: "缓冲区面积(亩)", parameter: "bufferArea" },
        { name: "审批面积(亩)", parameter: "approvedArea" },
        { name: "批建时间", parameter: "approvedTime" },
        { name: "调查人", parameter: "investigator" },
      ],
    };
  },
  ...
  methods: {
    ...
    // 列表数据查询
    getList() {
      naturalReserveApi("page", "post", this.queryData)
        .then((res) => {
    ...
    async handleMoveSelectClick(type) {
      ...
      this.$nextTick(() => {
        this.$refs["naturalReserveEdit"].updateDetails();
      });
    },
    ...
    async tableCurrentChange(currentRow) {
      ...
      if (this.isScreenCutting) {
        ...
        this.$nextTick(() => {
          this.$refs["naturalReserveEdit"].initDetails();
        });
      }
    },
    ...
    downExcelClick() {
      ...
      naturalReserveApi("export_all/excel", "post", data).then((res) => {
    ...
    getDeleteFunction(data) {
      ...
      this.$confirm("确定删除？", "提示", {
        ...
        .then(() => {
          naturalReserveApi("delete", "post", ids).then((res) => {
            this.$message({
              type: "success",
              message: "删除成功",
            });
