      ...
      <div class="dialog-content-left" ref="dialogContentLeft">
        ...
        >
          ...
          >
            <dictionary-select
              v-if="dialogtitle != '详情'"
              v-model.trim="formDetailsDialog.carType"
            ...
            <div v-else>
              <dictionary-text
                ...
                :value="formDetailsDialog.carType"
              />
            </div>
          </el-form-item>
          ...
          >
            <dictionary-select
              v-if="dialogtitle != '详情'"
              v-model.trim="formDetailsDialog.catStatus"
            ...
            <div v-else>
              <dictionary-text
                ...
                :value="formDetailsDialog.catStatus"
              />
            </div>
          </el-form-item>
          ...
          <el-form-item
            ...
            prop="vehicleEquipmentDetails"
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.vehicleEquipmentDetails"
              type="textarea"
              placeholder="请输入车载设备详情"
            />
            <div v-else>{{ formDetailsDialog.vehicleEquipmentDetails }}</div>
          </el-form-item>
    ...
    return {
      ...
      formRules: {
        ...
        boundaryLine: [
          {
            required: true,
            message: "请输入空间位置/车辆经常停放的位置",
            trigger: "blur",
          },
        ],
        name: [{ required: true, message: "请输入名称", trigger: "blur" }],
        address: [
          { required: true, message: "请输入停放地址", trigger: "blur" },
        ],
        carType: [{ required: true, message: "请选择车型", trigger: "change" }],
        catStatus: [
          { required: true, message: "请选择车辆情况", trigger: "change" },
        ],
        responsible: [
  ...
  methods: {
    ...
    getDetailsFunction(queryData, listdata) {
      return new Promise((resolve, reject) => {
        firetruckApi("detail", "get", { id: queryData.id })
          .then((res) => {
            let data = {};
    ...
    handleSubmitClick() {
      this.$refs["formDetailsDialog"].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogtitle == "新增") {
            firetruckApi("create", "post", this.formDetailsDialog)
              .then((res) => {
                this.$message({
                  type: "success",
                  message: "新增成功",
