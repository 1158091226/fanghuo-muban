      ...
      <div class="dialog-content-left" ref="dialogContentLeft">
        ...
        >
          ...
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.loadingEquipmentCondition"
              type="textarea"
              placeholder="请输入装载设备情况"
            />
            <div v-else>{{ formDetailsDialog.loadingEquipmentCondition }}</div>
          </el-form-item>
    ...
    handleSubmitClick() {
      this.$refs["formDetailsDialog"].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogtitle == "新增") {
            colApi("create", "post", this.formDetailsDialog)
              .then((res) => {
                this.$message({
                  type: "success",
                  message: "新增成功",
