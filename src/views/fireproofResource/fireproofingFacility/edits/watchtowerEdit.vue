  ...
  <div class="list-details">
    ...
    <div class="details-content-box">
      ...
      >
        ...
        >
          <el-input
            v-if="detailsType != '详情'"
            v-model="formDetails.buildAltitude"
            style="width: 100%"
            placeholder="请输入建设海拔(米)"
            class="filter-item"
            clearable
          />
          <div v-else>{{ formDetails.buildAltitude }}</div>
        </el-form-item>
        <el-form-item
          label="建设时间"
          :class="[
        ...
        >
          <el-date-picker
            v-if="detailsType != '详情'"
            v-model="formDetails.buildTime"
            type="date"
            placeholder="请选择建设时间"
            style="width: 100%"
            value-format="yyyy-MM-dd"
          />
          <div v-else>{{ formDetails.buildTime }}</div>
        </el-form-item>
        ...
        >
          <el-input
            v-if="detailsType != '详情'"
            v-model="formDetails.towerHeigth"
            style="width: 100%"
            placeholder="请输入塔体总高度(米)"
            class="filter-item"
            clearable
          />
          <div v-else>{{ formDetails.towerHeigth }}</div>
        </el-form-item>
        <el-form-item
          label="观测面积(亩)"
          :class="[
        ...
        >
          <el-input
            v-if="detailsType != '详情'"
            v-model="formDetails.observedArea"
            style="width: 100%"
            placeholder="请输入观测面积(亩)"
            class="filter-item"
            clearable
          />
          <div v-else>{{ formDetails.observedArea }}</div>
        </el-form-item>
        <el-form-item
          label="总体有效瞭望覆盖面积(亩)"
          :class="[
          ...
          prop="overallEffectiveLookoutArea"
        >
          <el-input
            v-if="detailsType != '详情'"
            v-model="formDetails.overallEffectiveLookoutArea"
            style="width: 100%"
            placeholder="请输入总体有效瞭望覆盖面积(亩)"
            class="filter-item"
            clearable
          />
          <div v-else>{{ formDetails.overallEffectiveLookoutArea }}</div>
        </el-form-item>
        <el-form-item
          label="装载设备"
          :class="[
          ...
          prop="loadingEquipment"
        >
          <el-input
            v-if="detailsType != '详情'"
            v-model="formDetails.loadingEquipment"
            type="textarea"
            placeholder="请输入装载设备"
          />
          <div v-else>{{ formDetails.loadingEquipment }}</div>
        </el-form-item>
        ...
        >
          <el-input
            v-if="detailsType != '详情'"
            v-model="formDetails.staffing"
            type="textarea"
            placeholder="请输入人员配备情况"
          />
          <div v-else>{{ formDetails.staffing }}</div>
        </el-form-item>
        ...
        >
          <dictionary-select
            v-if="detailsType != '详情'"
            v-model.trim="formDetails.towerStatus"
        ...
        >
          <el-input
            v-if="detailsType != '详情'"
            v-model="formDetails.mgmtUnit"
            style="width: 100%"
            placeholder="请输入管护单位"
            class="filter-item"
            clearable
          />
          <div v-else>{{ formDetails.mgmtUnit }}</div>
        </el-form-item>
        <el-form-item
          label="负责人"
          :class="[
    ...
    <div v-show="isShowHandle" class="details-btn-box">
      ...
      <div v-if="detailsType == '详情'" class="btn-right">
        ...
        <el-button
          v-permission="['fireprot:watchtower:update']"
          type="primary"
          class="right-btn"
  ...
  methods: {
    ...
    detailsSaveClick() {
      this.$refs["formDetails"].validate((valid) => {
        if (valid) {
          ...
          watchtowerApi("update", "post", data)
            .then((res) => {
              this.$message({
                type: "success",
                message: "修改成功",
    ...
    getDetailsFunction(queryData, listdata) {
      return new Promise((resolve, reject) => {
        watchtowerApi("detail", "get", { id: queryData.id })
          .then((res) => {
            let data = {};
