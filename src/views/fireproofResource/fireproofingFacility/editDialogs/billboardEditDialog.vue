      ...
      <div class="dialog-content-left" ref="dialogContentLeft">
        ...
        >
          ...
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.producer"
              style="width: 100%"
              placeholder="请输入制作单位"
              class="filter-item"
              clearable
            />
            <div v-else>{{ formDetailsDialog.producer }}</div>
          </el-form-item>
          <el-form-item
            label="制作时间"
            :class="[
          ...
          >
            <el-date-picker
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.productionTime"
              type="date"
              placeholder="请选择制作时间"
              style="width: 100%"
              value-format="yyyy-MM-dd"
            />
            <div v-else>{{ formDetailsDialog.productionTime }}</div>
          </el-form-item>
    ...
    handleSubmitClick() {
      this.$refs["formDetailsDialog"].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogtitle == "新增") {
            billboardApi("create", "post", this.formDetailsDialog)
              .then((res) => {
                this.$message({
                  type: "success",
                  message: "新增成功",
