  ...
  <div class="app-list-page" v-loading="requestLoading">
    ...
    <div class="screen-box">
      ...
      <div class="screen-box-input">
        ...
        <div class="screen-box-item">
          <div class="item-text">编号</div>
          <el-input
            v-model.trim="query.uuid"
            style="flex: 1"
            placeholder="请输入编号"
            class="filter-item"
            clearable
        ...
        <div class="screen-box-item">
          <div class="item-text">建设类型</div>
          <dictionary-select
            v-model.trim="query.constructionType"
      ...
      <div class="screen-box-button">
        <el-popover placement="bottom" width="480" trigger="click">
          ...
          <div slot="reference" style="display: flex; align-items: center">
            <div v-permission="['fireprot:isolation:more']" class="text1">
              更多
        ...
        <el-button
          type="primary"
          class="btn"
          v-permission="['fireprot:isolation:query']"
    ...
    <div class="operation-box">
      <div class="operation-box-left">
        <el-button
          v-permission="['fireprot:isolation:create']"
          class="btn1"
          type="primary"
          icon="el-icon-addnew"
          plain
        ...
        >
        <el-button
          v-permission="['fireprot:isolation:delete']"
          class="btn2"
          type="danger"
        ...
        >
        <el-button
          v-permission="['fireprot:isolation:import']"
          class="btn3"
        ...
        >
        <el-button
          v-permission="['fireprot:isolation:export']"
          class="btn4"
        ...
        >
        <el-button
          v-permission="['fireprot:isolation:map-plus']"
          class="btn5"
          icon="el-icon-earth"
          plain
          >一张图</el-button
      ...
      <div class="operation-box-right">
        ...
        >
          ...
          <div
            slot="reference"
            v-permission="['fireprot:isolation:column-filter']"
        ...
        <div
          v-permission="['fireprot:isolation:layout']"
          style="display: flex; align-items: center"
    ...
    <div class="list-map-box" :class="[isMoveBox ? 'not-select' : '']">
      ...
      >
        ...
        >
          ...
          <el-table-column
            v-if="checkedTh.includes('boundaryLine')"
            prop="boundaryLine"
            label="隔离带范围/地理信息"
            min-width="120"
            show-overflow-tooltip
          ...
          <el-table-column
            v-if="checkedTh.includes('smallPlaceName')"
            prop="smallPlaceName"
            label="小地名"
            min-width="120"
            show-overflow-tooltip
          ...
          <el-table-column
            v-if="checkedTh.includes('strongShuZhong')"
            prop="strongShuZhong"
            label="所在林分的优势树种"
            min-width="120"
            show-overflow-tooltip
          ...
          <el-table-column
            v-if="checkedTh.includes('riskLevel')"
            prop="riskLevel"
            label="所在风险等级"
            min-width="120"
            show-overflow-tooltip
          ...
          <el-table-column
            v-if="checkedTh.includes('length')"
            prop="length"
            label="长度(千米)"
            min-width="120"
            show-overflow-tooltip
          ...
          <el-table-column
            v-if="checkedTh.includes('constructionArea')"
          ...
          <el-table-column
            v-if="checkedTh.includes('constructionType')"
          ...
          >
            <template slot-scope="scope">
              <dictionary-text
                ...
                :value="scope.row.constructionType"
              />
            </template>
          </el-table-column>

          <el-table-column
            v-if="checkedTh.includes('buildType')"
            prop="buildType"
            label="营造模式"
            min-width="120"
            show-overflow-tooltip
          ...
          <el-table-column
            v-if="checkedTh.includes('forestZoneLevel')"
          ...
          >
            <template slot-scope="scope">
              <dictionary-text
                ...
                :value="scope.row.forestZoneLevel"
              />
            </template>
          </el-table-column>

          <el-table-column
            v-if="checkedTh.includes('funcType')"
            prop="funcType"
            label="功能类型"
            min-width="120"
            show-overflow-tooltip
          >
            <template slot-scope="scope">
              <dictionary-text
                ...
                :value="scope.row.funcType"
              />
            </template>
          </el-table-column>

          <el-table-column
            v-if="checkedTh.includes('fireProtShuZhong')"
          ...
          <el-table-column
            v-if="checkedTh.includes('buildStartTime')"
            prop="buildStartTime"
            label="开始建设时间"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('buildEndTime')"
            prop="buildEndTime"
            label="结束建设时间"
            min-width="120"
            sortable="custom"
          ...
          <el-table-column
            v-if="checkedTh.includes('applicant')"
            prop="applicant"
            label="申请人"
            min-width="120"
            show-overflow-tooltip
          ...
          <el-table-column
            v-if="checkedTh.includes('applicationTime')"
...
import IsolationEditDialog from "./editDialogs/isolationEditDialog.vue";
import IsolationEdit from "./edits/isolationEdit.vue";
...
export default {
  ...
  data() {
    return {
      ...
      checkAll: false,
      checkedTh: fireproofDict("fireprot_isolation").showColumn,
      isIndeterminate: true,
      // 表格表头列表
      tableListTitle: [
        { name: "区划代码", parameter: "areaCode" },
        { name: "编号", parameter: "uuid" },
        { name: "隔离带范围/地理信息", parameter: "boundaryLine" },
        { name: "名称", parameter: "name" },
        { name: "小地名", parameter: "smallPlaceName" },
        { name: "土地类型", parameter: "diLei" },
        { name: "所在林分的优势树种", parameter: "strongShuZhong" },
        { name: "所在风险等级", parameter: "riskLevel" },
        { name: "长度(千米)", parameter: "length" },
        { name: "宽度(米)", parameter: "width" },
        { name: "建设区域", parameter: "constructionArea" },
        { name: "建设类型", parameter: "constructionType" },
        { name: "营造模式", parameter: "buildType" },
        { name: "林带等级", parameter: "forestZoneLevel" },
        { name: "功能类型", parameter: "funcType" },
        { name: "防火树种", parameter: "fireProtShuZhong" },
        { name: "开始建设时间", parameter: "buildStartTime" },
        { name: "结束建设时间", parameter: "buildEndTime" },
        { name: "状态", parameter: "status" },
        { name: "隔离带类型", parameter: "type" },
        { name: "申请人", parameter: "applicant" },
        { name: "申请时间", parameter: "applicationTime" },
  ...
  methods: {
    ...
    // 列表数据查询
    getList() {
      isolationApi("page", "post", this.queryData)
        .then((res) => {
    ...
    async handleMoveSelectClick(type) {
      ...
      this.$nextTick(() => {
        this.$refs["isolationEdit"].updateDetails();
      });
    },
    ...
    async tableCurrentChange(currentRow) {
      ...
      if (this.isScreenCutting) {
        ...
        this.$nextTick(() => {
          this.$refs["isolationEdit"].initDetails();
        });
      }
    },
    ...
    downExcelClick() {
      ...
      isolationApi("export_all/excel", "post", data).then((res) => {
    ...
    getDeleteFunction(data) {
      ...
      this.$confirm("确定删除？", "提示", {
        ...
        .then(() => {
          isolationApi("delete", "post", ids).then((res) => {
            this.$message({
              type: "success",
              message: "删除成功",
            });
