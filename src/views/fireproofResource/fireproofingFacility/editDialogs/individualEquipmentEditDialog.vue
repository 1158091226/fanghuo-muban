      ...
      <div class="dialog-content-left" ref="dialogContentLeft">
        ...
        >
          ...
          <el-form-item
            ...
            prop="deviceStorageLocation"
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.deviceStorageLocation"
              style="width: 100%"
              placeholder="请输入存放位置"
              class="filter-item"
              clearable
            />
            <div v-else>{{ formDetailsDialog.deviceStorageLocation }}</div>
          </el-form-item>
          <el-form-item
            label="设备详情"
            :class="[
          ...
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.equipmentDetails"
              type="textarea"
              placeholder="请输入设备详情"
            />
            <div v-else>{{ formDetailsDialog.equipmentDetails }}</div>
          </el-form-item>
          ...
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.userName"
              style="width: 100%"
              placeholder="请输入使用人员"
              class="filter-item"
              clearable
            />
            <div v-else>{{ formDetailsDialog.userName }}</div>
          </el-form-item>
          <el-form-item
            label="使用人员电话"
            :class="[
          ...
          >
            <el-input
              v-if="dialogtitle != '详情'"
              v-model="formDetailsDialog.userTel"
              style="width: 100%"
              placeholder="请输入使用人员电话"
              class="filter-item"
              clearable
            />
            <div v-else>
              <div v-if="isShowuserTel">
                {{ isShowSecretFunction(formDetailsDialog.userTel, 3, 7)
                }}<el-button
                  size="mini"
                  class="ml-10"
                  @click="isShowuserTel = !isShowuserTel"
                  >查看</el-button
                >
              </div>
              <div v-else>
                {{ formDetailsDialog.userTel
                }}<el-button
                  size="mini"
                  class="ml-10"
                  @click="isShowuserTel = !isShowuserTel"
  ...
  methods: {
    ...
    getDetailsFunction(queryData, listdata) {
      return new Promise((resolve, reject) => {
        individualEquipmentApi("detail", "get", { id: queryData.id })
          .then((res) => {
            let data = {};
    ...
    // 查看详情的时候把数据都脱敏
    handleIsShow() {
      this.isShowuserTel = true; // 控制敏感数据查看
    ...
    handleSubmitClick() {
      this.$refs["formDetailsDialog"].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogtitle == "新增") {
            individualEquipmentApi("create", "post", this.formDetailsDialog)
              .then((res) => {
                this.$message({
                  type: "success",
                  message: "新增成功",
            ...
            individualEquipmentApi("update", "post", data)
              .then((res) => {
                this.$message({
                  type: "success",
                  message: "修改成功",
