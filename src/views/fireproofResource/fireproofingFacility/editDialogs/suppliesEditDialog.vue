        ...
        <div class="dialog-content-left" ref="dialogContentLeft">
          ...
          >
            ...
            >
              <el-input
                v-if="dialogtitle != '详情'"
                v-model="formDetailsDialog.isDelete"
                style="width: 100%"
                placeholder="请输入删除状态（1-已删除：0-未删除）"
                class="filter-item"
                clearable
              />
              <div v-else>{{ formDetailsDialog.isDelete }}</div>
            </el-form-item> -->
            <el-form-item
              label="类型名称"
              :class="[
            ...
            >
              <el-input-number
                v-if="dialogtitle != '详情'"
                v-model="formDetailsDialog.amount"
                style="width: 100%"
                placeholder="请输入数量"
                :controls="false"
              />
              <div v-else>{{ formDetailsDialog.amount }}</div>
            </el-form-item>
            ...
            >
              <dictionary-select
                v-if="dialogtitle != '详情'"
                v-model.trim="formDetailsDialog.unit"
                type="supplies_unit"
                :show-code="false"
                :placeholder="'请选择单位'"
                class="filter-item"
            ...
            >
              <el-input
                v-if="dialogtitle != '详情'"
                v-model="formDetailsDialog.status"
                style="width: 100%"
                placeholder="请输入状态"
                class="filter-item"
                clearable
              />
              <div v-else>{{ formDetailsDialog.status }}</div>
            </el-form-item> -->
            <el-form-item
              label="图片"
              :class="[
            ...
            >
              <dictionary-select
                v-if="dialogtitle != '详情'"
                v-model.trim="formDetailsDialog.statusCode"
              ...
              <div v-else>
                <dictionary-text
                  ...
                  :value="formDetailsDialog.statusCode"
                />
              </div>
            </el-form-item>
    ...
    handleSubmitClick() {
      this.$refs["formDetailsDialog"].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogtitle == "新增") {
            suppliesApi("create", "post", this.formDetailsDialog)
              .then((res) => {
                this.$message({
                  type: "success",
                  message: "新增成功",
