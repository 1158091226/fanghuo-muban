...
export default {
  ...
  methods: {
    ...
    async getTreeData() {
      ...
      this.handleTlMapTypeList(this.mapTypeList);
      this.treeData = [...this.treeData];
    },
    // TlMapTypeList：data根据parentId找到父级，并塞入childList，并移除自身
    handleTlMapTypeList(data) {
      let arr = [];
      data.forEach((item) => {
        item.childList = [];
        let parentId = item.parentId;
        let parent = data.find((it) => it.id == parentId);
        if (parent) {
          parent.childList = parent.childList || [];
    ...
    mapGetDetail(data, feature, coordinate) {
      console.log(data, feature, coordinate);
      //点击图层回调事件
      let mapType = "";
      let query = {};
      ...
      for (let index = 0; index < data.length; index++) {
        ...
        if (this.clearTypeList.includes(item.type)) {
          ...
          obj.getData = async function (type, active) {
            ...
            } else if (obj.mapType == 2 || obj.mapType == 4) {
              obj.list = [{}];
            }
